...
let bosszone = bossenemy.getBoundingClientRect();
  ...
  if( playerzone.right > bosszone.left &&
    playerzone.left < bosszone.right &&
    playerzone.bottom > bosszone.top &&
    playerzone.top < bosszone.bottom )
     {
      console.log('scared?')
      gameend();
    }
...
// boss
    ...
    let bossenemy = document.getElementById('negativeplayer')
  function bossenter() {
    console.log(`boss entering`)
    bossmusic.play();
    ...
    clearInterval(fortheblocks);
    clearTimeout(forcheckwave)
    bossenemy.classList.add('bossenter')
    bossenemy.addEventListener('animationend' , () => {
      bossenemy.classList.remove('bossenter');
      move()
  }, { once: true })
  
  }
  function move() {
     bossenemy.classList.add('bossmove')
     console.log(`moving`)
     bossenemy.addEventListener('animationend' , () => {
        bossenemy.classList.remove('bossmove');
        aftermove()
  },{ once: true })
  }
  ...
  function aftermove() {
     ...
     bossenemy.classList.add('bossjump')
     console.log(`jumping`)
     bossenemy.addEventListener('animationend', () => {
        bossenemy.classList.remove('bossjump');
        afterjump()
     },{ once: true })
  ...
  function afterjump() {
     console.log(`landing`)
     bossenemy.classList.add('bossland');
     bossenemy.addEventListener('animationend' ,() => {
     bossenemy.classList.remove('bossland')
     bossenemy.style.display='none'
     bossmusic.pause();
...
function checkboss () {
  if(Displayscore > 1500) {
    bossenter()
    bossenemy.style.display ='block'
    return;
  }
  forcheckboss = setTimeout(checkboss , 50)
}
